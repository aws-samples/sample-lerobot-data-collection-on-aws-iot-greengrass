...
- `com.groot.kvs-webrtc-p2p` — color camera → single capture tee'd to KVS WebRTC P2P master (sub-second live) + kvssink recording (HLS replay). Low-latency alternative to kvs-webrtc-ingest.
...
## Web UI deployment
- Replace the S3 web pages (`live-p2p.html`, `multiviewer.html`; text/html) + CloudFront invalidation (`/live-p2p.html`, `/multiviewer.html`, `/`).

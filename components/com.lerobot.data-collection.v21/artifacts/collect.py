...
# Inter-episode reset window (lerobot --dataset.reset_time_s). During this gap
# lerobot logs "Reset the environment" and waits so the operator can reset the
# scene before the next / re-recorded episode. Surfaced to the web UI as a
# per-second countdown ("reset time remaining N").
...
class Controller:
    def __init__(self):
        ...
        self._last_rerecord = 0.0    # wall-clock of last 'discard'(=FIFO rerecord) request
        self._reset_cd_active = False # True while a reset-countdown thread runs
        self._rec_seq = 0            # increments on every actual 'Recording episode' start
    ...
    def _handle(self, payload):
        ...
        if action == "start":
            ...
            # "Discard" = drop ONLY the CURRENT (in-progress) episode and
            # re-record the SAME episode number, WITHOUT ending the session
            # (lerobot rerecord_episode). e.g. recording episode 5 -> Discard ->
            # the just-started episode 5 is thrown away and episode 5 starts
            # recording again. Sent via the control FIFO ('rerecord'); the
            # patched lerobot control_utils sets rerecord_episode + exit_early,
            # then record.py calls dataset.clear_episode_buffer() and re-records
            # the same index (recorded_episodes is NOT incremented). A short
            # reset_time_s gap ('Reset the environment') precedes the retake.
            # No MIN_EP_SECS guard is needed: rerecord clears the buffer instead
            # of calling add_episode(), so a 0-frame retake is safe.
            self._last_rerecord = time.time()
            self._send_control("rerecord")
            return
        elif action == "discardSession":
            # Full-session discard: hard-stop the container, no upload.
            # (Kept for completeness; the web UI Discard button uses "discard"
            #  above = per-episode re-record.)
    ...
    def _parse_progress(self, line):
        ...
        try:
            ...
            if m:
                ...
                # Detect a re-record (Discard button): lerobot re-emits
                # 'Recording episode N' with the SAME index because
                # recorded_episodes was NOT incremented (clear_episode_buffer).
                # Drop the discarded take's KVS window instead of leaving a
                # duplicate window for the same episode number.
                is_rerecord = bool(self._episodes and
                                   self._episodes[-1].get("episode") == ep)
                if is_rerecord:
                    self._episodes.pop()          # remove discarded take's window
                elif self._episodes and self._episodes[-1].get("end") is None:
                    # Normal transition: close the previous episode's KVS window.
                ...
                # Report how the previous take ended: a re-record (Discard),
                # a MANUAL "Stop & Save" (a 'next' during the previous episode),
                # or an automatic TIMEOUT (episode_time_s elapsed).
                if is_rerecord:
                    print(f"[RERECORD] episode {ep} discarded — re-recording episode {ep}")
                elif had_prev and prev_start:
                ...
                # Emit a status on EVERY actual recording start (including ep1
                # and re-record retakes, which keep the SAME episode number) so
                # an external orchestrator can trigger precisely off real
                # episode starts. recSeq increments once per 'Recording episode'.
                self._rec_seq += 1
            ...
            # Inter-episode reset window: lerobot logs "Reset the environment"
            # at the start of the reset_time_s gap (between episodes, and before
            # a re-recorded episode after Discard). Surface a per-second
            # countdown to the web UI so the reset period is visible.
            if "Reset the environment" in line:
                print(f"[RESET] reset phase started ({RESET_TIME_S}s)")
                threading.Thread(target=self._reset_countdown,
                                 args=(RESET_TIME_S,), daemon=True).start()
        except Exception:
            pass

    def _reset_countdown(self, seconds):
        """Publish a per-second reset countdown to the web UI during lerobot's
        inter-episode reset window. Reuses TOPIC_STATUS (no new topic / access
        control) with an extra 'resetRemaining' field; the UI logs
        "reset time remaining N". Guarded so overlapping resets don't stack."""
        if self._reset_cd_active:
            return
        self._reset_cd_active = True
        try:
            for i in range(int(seconds), 0, -1):
                # Stop early if the container is gone (session ended/crashed).
                if not (self._proc and self._proc.poll() is None):
                    break
                ...
                time.sleep(1)  # nosemgrep: arbitrary-sleep -- intentional 1s countdown tick
        finally:
